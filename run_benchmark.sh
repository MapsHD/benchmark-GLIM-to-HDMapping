...
# run_benchmark.sh - Reusable benchmark runner for GLIM
...
#   From main repo: ./methods/benchmark-GLIM-to-HDMapping/run_benchmark.sh avia /path/to/data/
...
docker run --rm -it \
    ...
    ghcr.io/mapshd/glim2hdmapping:latest \
    bash -c "source /test_ws/install/setup.sh && rm -rf /data/hdmapping && ros2 run glim-to-hdmapping listener /data/*.mcap /data/hdmapping"
